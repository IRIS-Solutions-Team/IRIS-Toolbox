...
% yearly  Display time series one calendar year per row
%
% Syntax
% =======
%
...
function X = disp2D(Start, Data, Tab, Sep, Num2StrFunc)
    ...
    numOfYears = lastYear - firstYear + 1;
    ...
    if nx > 1
        swap = dates;
        dates = repmat(' ', nx*numOfYears, size(dates, 2));
        dates(1:nx:end, :) = swap;
    end
