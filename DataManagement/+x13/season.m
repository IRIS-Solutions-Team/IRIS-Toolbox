...
arguments
    ...
    specs.Series_CompType string { validate.mustBeScalarOrEmpty, validate.mustBeAnyString(specs.Series_CompType, ["add", "sub", "mult", "div", "none"]) } = string.empty(1, 0)
    ...
    specs.Series_Type string { validate.mustBeScalarOrEmpty, validate.mustBeAnyString(specs.Series_Type, ["stock", "flow"]) } = string.empty(1, 0)
    ...
    specs.Transform_Function string { validate.mustBeScalarOrEmpty, validate.mustBeAnyString(specs.Transform_Function, ["auto", "log", "sqrt", "inverse", "logistic", "none"]) } = string.empty(1, 0)
    specs.Transform_Power { validate.mustBeScalarOrEmpty, mustBeNumeric } = double.empty(1, 0)
    specs.Transform_Adjust string { validate.mustBeScalarOrEmpty, validate.mustBeAnyString(specs.Transform_Adjust, ["lom", "loq", "lpyear"]) } = string.empty(1, 0)
    ...
    specs.Estimate_Exact string { validate.mustBeScalarOrEmpty, validate.mustBeAnyString(specs.Estimate_Exact, ["arma", "ma", "none"]) } = string.empty(1, 0)
    ...
    specs.Force_Type string { validate.mustBeScalarOrEmpty, validate.mustBeAnyString(specs.Force_Type, ["none", "regress", "denton"]) } = string.empty(1, 0)
    ...
    specs.Force_Target string { validate.mustBeScalarOrEmpty, validate.mustBeAnyString(specs.Force_Target, ["original", "caladjust", "permprioradj", "both"]) } = string.empty(1, 0)
    ...
    specs.X11Regression_OutlierMethod string { validate.mustBeScalarOrEmpty, validate.mustBeAnyString(specs.X11Regression_OutlierMethod, ["addone", "addall"]) } = string.empty(1, 0)
