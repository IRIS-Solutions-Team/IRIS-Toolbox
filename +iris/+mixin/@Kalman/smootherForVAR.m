...
function ...
    [X2, Px2, E, U, Y2, Py2, inxY, Y0, F, Y1, Py1] ...
    = smootherForVAR(this, A, B, K, Z, D, Omg, Sgm, y, E, x0, P0, settings)
    ...
    allObs = settings.allObs;
    invFunc = settings.invFunc;
    ...
    isSgm = ~isempty(Sgm) && any(Sgm(:));

    if allObs
        PP = BOmgBt;
        fullFixedF = Z*PP*Z';
        if isSgm
            fullFixedF = fullFixedF + Sgm;
        end
        fullFixedFi = invFunc(fullFixedF);
        fullFixedF = ensureSymmetric(fullFixedF);
        fullFixedFi = ensureSymmetric(fullFixedFi);
    end

    % __Prediction and updating steps__

    % First prediction step
    inxNonzero = any(A~=0, 1);
    X2(1:nx, 1) = A(:, inxNonzero)*x0(inxNonzero) + Be(:, 1);
    ...
    for t = 1 : numPeriods
        ...
        if t < numPeriods
            X2(1:nx, t+1) = A(:, inxNonzero)*X2(inxNonzero, t) + Be(:, t+1);
            if ~isEmptyK
    ...
    lastObs = max([0, find(any(inxY, 1), 1, 'last')]);
    ...
    return
        ...
        function here_ahead()
            ...
            for kk = 2 : min(ahead, numPeriods-t+1)
                x(1:nx, kk) = A(:, inxNonzero)*x(inxNonzero, kk-1);
                if ~isEmptyK
