...
if isempty(varargin)
    ...
    [~, ~, a.Dynamic] = model.Pairing.getAutoexog(this.Pairing.Autoexog.Dynamic, this.Quantity);
    [~, ~, a.Steady] = model.Pairing.getAutoexog(this.Pairing.Autoexog.Steady, this.Quantity);
    varargout{1} = a;
else
    a = varargin{1};
...
return
    ...
    function p = setType(a, p, type)
            ...
            lsEndog = lsEndog(:).';
            p = model.Pairing.setAutoexog(p, type, this.Quantity, lsExog, lsEndog);
    end
end
